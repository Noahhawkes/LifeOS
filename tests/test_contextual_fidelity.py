...
def test_8_public_witness_message_requires_verification_boundary():
    # Public witness environments map down to standard non-smoothed raw validation frameworks
...
def test_11_playful_lore_is_allowed_only_when_labeled():
    ...
    assert rule_check["approved"] is False  # Rejects mythic descriptors outside playful_lore context configuration
...
def test_13_personality_never_overrides_truth():
    ...
    assert rule_check["approved"] is False  # Fails due to narrative smoothing indicators ('surely', 'probably')
...
def test_14_apply_fidelity_rules_rejects_unsupported_certainty():
    ...
    assert rule_check["approved"] is False  # Rejects since it lacks 'i do not know' or structural gap signals
...
def test_15_apply_fidelity_rules_rejects_fictionalized_serious_claims():
    ...
    # Prohibit explicitly forbidden 100% fidelity metric assertions
...
def test_17_module_does_not_write_files():
    # Code-level architectural validation: the module contains absolutely zero open, write, or system manipulation IO layers
    import inspect
